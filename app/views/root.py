...
def sign_in():
    ...
    if form.validate_on_submit():
        ...
        if login.confirm(form.email.data, form.password.data):
            ...
            redis_client.hset(f"active_users/{user.username}", "active", "true")
            redis_client.hset(f"active_users/{user.username}", "campus", 1)
...
def sign_out():
    ...
    redis_client.delete(f"active_users/{current_user.username}")
    logout_user()
    return "signed out"
